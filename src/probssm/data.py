...
def load_COVID_data(country, num_data_points=None, include_death=True):
    ...
    SIR_data = np.stack([np.array(t, dtype=np.float64) for t in [S_data, I_data, R_data]]
    ).T
    ...
    SIR_data = SIR_data[:num_data_points]
    SIRD_data.setflags(write=False)
    SIR_data.setflags(write=False)
    
    if include_death:
        return (day_zero, date_range_x, SIRD_data, population)
    else:
        return (day_zero, date_range_x, SIR_data, population)
