...
def sird_jac_beta(t, y, beta, gamma, eta, population_count):
    ...
    return d_dbeta
#--------------------------SIR Model----------------------------------#
def sir_rhs(t, y, beta, gamma, population_count):
    """RHS for SIR model"""
...
def sir_jac_x(t, y, beta, gamma, population_count):
    """Jacobian for SIR model w.r.t. the state"""
    S, I, R = y
    d_dS = np.array(
        [-beta * I / population_count, -beta * S / population_count, 0.0]
    )
    d_dI = np.array(
        [
            beta * I / population_count,
    ...
    d_dR = np.array([0.0, gamma, 0.0])
    jac_matrix = np.array([d_dS, d_dI, d_dR])
    return jac_matrix


def sir_jac_beta(t, y, beta, gamma, population_count):
