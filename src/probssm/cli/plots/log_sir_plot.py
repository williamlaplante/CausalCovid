...
def main():
    ...
    posterior = np.load(log_dir / "smoothing_posterior_first.npz")
    data = np.load(log_dir / "data_info.npz")
    with open(log_dir / "info.json", "r") as file_handle:
        info = json.load(file_handle)

    skip = int(1/info["filter_step_size"])
    mult = int(info["x_process_ordint"])+1
    
    fig, axs = plt.subplots(1,3,figsize=(16,4))
    for i, label in enumerate(["Susceptible", "Infected", "Recovered"]):
        axs[i].plot(np.exp(posterior["means"][:, mult*i])[::skip], label="Prediction")
        axs[i].plot(data["sir_data"][:, i], label="Data")
        axs[i].set_title(label)
        axs[i].legend()
    fig.savefig(log_dir / "states.png")
    plt.close()
    return
